...
Variables can be substituted in any call and are denoted by wrapping the variable name in caret characters (`^`).
...
The substitution is a value substitution, not an object substitution. The variable is not "passed" to commands or scripts. The value the variable holds at that time is substituted into the call.
